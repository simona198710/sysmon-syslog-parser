...
def parse_log_entry(line):
    ...
    if 'DestinationIp' not in line:
        return None

    # Regex to capture the full path, destination IP, and destination port
    match = re.search(r'Image:\s*(.+?)\s+User:', line)
    ip_match = re.search(r".*?DestinationIp:\s*(?P<ip>[a-fA-F0-9:\.]+)", line)
    port_match = re.search(r'DestinationPort:\s*(\d+)', line)
    hostname_match = re.search(r'DestinationHostname:\s*([^\s]+)?', line)
    ...
    if match and ip_match and port_match:
        ...
        if dest_hostname == "DestinationPort:":
            dest_hostname = ""
